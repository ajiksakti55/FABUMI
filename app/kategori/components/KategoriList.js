...
export default function CategoryList({ kategori = [], onEdit = () => {}, onDeleted = () => {} }) {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto mt-6 space-y-4">
      ...
      {parents.length === 0 ? (
        <div className="text-gray-600 text-center bg-white/80 p-6 rounded-xl shadow-sm border border-gray-100 text-sm sm:text-base">
        ...
        parents.map((p) => (
          <div
            key={p.id}
            className="bg-white/80 backdrop-blur-md border border-gray-100 rounded-xl shadow-sm hover:shadow-md transition-all duration-300 p-4"
          >
            {/* Kategori Utama */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div>
                <div className="font-semibold text-gray-800 text-base sm:text-lg">{p.name}</div>
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => onEdit(p)}
                  className="px-3 py-1.5 text-xs sm:text-sm rounded-md border border-gray-300 hover:bg-gray-100 text-gray-700 transition-all duration-200"
                >
                  ✏️ Edit
                </button>
                <button
                  ...
                  className="px-3 py-1.5 text-xs sm:text-sm rounded-md border border-red-300 hover:bg-red-50 text-red-600 transition-all duration-200"
                >
                  {deletingId === p.id ? "⏳..." : "🗑️ Hapus"}
                </button>
              </div>
            </div>
              ...
              {childrenMap[p.id] ? (
                <ul className="space-y-1">
                  {childrenMap[p.id].map((c) => (
                    <li
                      key={c.id}
                      className="flex flex-col sm:flex-row sm:items-center justify-between bg-gray-50 rounded-lg px-3 py-2 hover:bg-gray-100 transition-all duration-200 text-sm"
                    >
